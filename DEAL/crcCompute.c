...
uint16_t CRC_Compute16( uint8_t* data, uint32_t data_length)
{
	...
	crchl = 0x00 ;
	for ( ii = 0x00 ; ii < data_length ; ii ++ )
	{
		da= ( crchl / 256 ) ;              // 以8位二进制数的形式暂存CRC的高8位 
		crchl <<= 0x08 ;                   // 左移8位，相当于CRC的低8位乘以  
		crchl ^= crcTable [ da ^ data[ii]];// 高8位和当前字节相加后再查表求CRC ，再加上以前的CRC 
	}
	return crchl;
}
...
uint16_t CRC_Compute2( uint8_t* data1, uint32_t length1, uint8_t* data2 ,uint32_t length2 )
{
	...
	crchl = 0x00 ;
	for ( ii = 0x00 ; ii < length1 ; ii ++ )
	{
		da = ( crchl / 256 ) ;               // 以8位二进制数的形式暂存CRC的高8位 
		crchl <<= 0x08 ;                     // 左移8位，相当于CRC的低8位乘以  
		crchl ^= crcTable [ da ^ data1[ii]] ;// 高8位和当前字节相加后再查表求CRC ，再加上以前的CRC 
	}
	for ( ii = 0x00 ; ii < length2 ; ii ++ )
	{
		da = ( crchl / 256 ) ;               // 以8位二进制数的形式暂存CRC的高8位 
		crchl <<= 0x08 ;                     // 左移8位，相当于CRC的低8位乘以  
		crchl ^= crcTable [ da ^ data2[ii]]; // 高8位和当前字节相加后再查表求CRC ，再加上以前的CRC 
	}
	return crchl;
}
