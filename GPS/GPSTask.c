...
//       2,没有找到同步字符
//       3,接收到NACK应答
u8 Ublox_Cfg_Ack_Check(void)//todo:判断应答
{			 
	uint16_t length = 0;
	uint8_t err,i;
	uint8_t *ptrGPSRece = NULL;
	ptrGPSRece = OSQPend(receGPSQ,250,&err);//等待接收到应答   
	if(err == OS_ERR_TIMEOUT)
		return 1;
	length = ptrGPSRece[0];         //此次接收到的数据长度 
	length <<= 8;
	length += ptrGPSRece[1];
...
u8 Ublox_Cfg_Cfg_Save(void)
{
	...
	GPSSendDatas((u8*)cfg_cfg,sizeof(_ublox_cfg_cfg));//发送数据给NEO-6M     
	for(i=0;i<6;i++)
...
u8 Ublox_Cfg_Msg(u8 msgid,u8 uart1set)
{
	...
	GPSSendDatas((u8*)cfg_msg,sizeof(_ublox_cfg_msg));//发送数据给NEO-6M   
	Mem_free(cfg_msg);
...
u8 Ublox_Cfg_Prt(u32 baudrate)
{
	...
	GPSSendDatas((u8*)cfg_prt,sizeof(_ublox_cfg_prt));//发送数据给NEO-6M   
	OSTimeDlyHMSM(0,0,0,200);				//等待发送完成 
...
u8 Ublox_Cfg_Tp(u32 interval,u32 length,signed char status)
{
	...
	GPSSendDatas((u8*)cfg_tp,sizeof(_ublox_cfg_tp));//发送数据给NEO-6M  
	Mem_free(cfg_tp);
	return Ublox_Cfg_Ack_Check();
}
//配置UBLOX NEO-6的更新速率	    
...
u8 Ublox_Cfg_Rate(u16 measrate,u8 reftime)
{
	...
	GPSSendDatas((u8*)cfg_rate,sizeof(_ublox_cfg_rate));//发送数据给NEO-6M 
	Mem_free(cfg_rate);
	return Ublox_Cfg_Ack_Check();
}
